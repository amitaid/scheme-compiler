...
def main(tests):
    ...
    only_parser.add_argument("-s", "--only-sections", help="Only run these test sections", type=int, nargs='*',
                             default=[])
...
tests.append(
    ("(append '(1 2) '(3 4) (list 5 6))", "(1 . (2 . (3 . (4 . (5 . (6 . ()))))))", 'append with multiple args'))
...
tests.append(
    ("(map + '(1 2 3) '(2 4 6) '(5 10 15))", "(8 . (16 . (24 . ())))", 'map on a lambda-simple with three arg lists'))
...
tests.append(
    ("(eq? (string->symbol \"hi\") 'hi)", '#f', 'eq between dynamic retrieval of a symbol from string and the symbol'))
tests.append(
    ("(eq? (symbol->string 'hi) \"hi\")", '#f', 'eq between dynamic retrieval of a string from symbol and the string'))
tests.append(
    ("(eq? (string->symbol \"HI\") 'hi)", '#t', 'eq between dynamic retrieval of a symbol from string and the symbol'))
tests.append(
    ("(eq? (symbol->string 'hi) \"HI\")", '#t', 'eq between dynamic retrieval of a string from symbol and the string'))
tests.append(
    ("(eq? (string->symbol (symbol->string 'hi)) 'hi)", '#t', 'symbol->string and string->symbol invariant test'))
tests.append(("(eq? 'hi \"hi\")", '#f', 'a symbol and the matching string are different objects'))
tests.append((
    "(eq? (string->symbol \"sss\") (string->symbol (make-string 3 #\s)))", "#t", 'eq of two different dynamic symbols'))
