...
EQ_ARGS_AMOUNT_OK:
 ...
 EQ_COMPARE:
  ...
  CMP(IND(R0), IMM(T_NIL));  // nil
  JUMP_EQ(EQ_ADRESS_COMPARE);
  CMP(IND(R0), IMM(T_BOOL)); // boolean
  JUMP_EQ(EQ_ADRESS_COMPARE);
  CMP(IND(R0), IMM(T_CHAR));  // char
  JUMP_EQ(EQ_VALUE_COMPARE_CHAR_INT);
  CMP(IND(R0), IMM(T_INTEGER)); // int
  JUMP_EQ(EQ_VALUE_COMPARE_CHAR_INT);
  CMP(IND(R0), IMM(T_FRACTION));
  JUMP_EQ(EQ_VALUE_COMPARE_FRACTION);
  CMP(IND(R0), IMM(T_STRING));  // string
  JUMP_EQ(EQ_ADRESS_COMPARE);
  CMP(IND(R0), IMM(T_SYMBOL));
  JUMP_EQ(EQ_VALUE_COMPARE_SYM);
  CMP(IND(R0), IMM(T_PAIR));  // pair
  JUMP_EQ(EQ_ADRESS_COMPARE);
  CMP(IND(R0), IMM(T_VECTOR)); // vector
  JUMP_EQ(EQ_ADRESS_COMPARE);
